...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for state in states:  #for each null terminal state
          predecessor = set()
          for state2 in states:
            for act in self.mdp.getPossibleActions(state2):
              for tran in self.mdp.getTransitionStatesAndProbs(state2,act):
                if tran[0] == state:
                  predecessor.add(state2)

          predecessors[c] = predecessor
          c = c + 1
          if self.mdp.isTerminal(state):
            self.values[state] = 0
            ...
            for act in self.mdp.getPossibleActions(state):
              if Qmax < self.getQValue(state, act):
                Qmax = self.getQValue(state, act)

              diff = self.getValue(state) - Qmax
        ...
        for i in range(self.iterations):
          if pQueue.isEmpty() == False:
            ...
            for pre in predecessors[i]:
              Qmax = -99999999999
              for act in self.mdp.getPossibleActions(s):
                if Qmax < self.getQValue(s, act):
                  Qmax = self.getQValue(s, act)

              d = self.getValue(s) - Qmax
